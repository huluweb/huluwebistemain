...
const MapWithSearch = () => {
  ...
  const getIcon = (type) => {
    switch(type) {
      ...
      case 'searched': return searchedLocationIcon;
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(async () => {
      ...
      const allSuggestions = [
        ...localMatches,
        ...geoResults.filter(geo => 
          !localMatches.some(local => local.name === geo.name)
        )
      ];
  ...
  const fetchRoutes = useCallback(async (fromPosition) => {
    ...
    try {
      const routePromises = ADDIS_LOCATIONS
        .filter(loc => loc.position[0] !== fromPosition[0] || loc.position[1] !== fromPosition[1])
        .map(async (location) => {
          try {
      ...
      setRoutes(results.filter(route => route !== null));
    } catch (err) {
  ...
  return (
    <div className="relative w-full h-[600px]">
      ...
      >
        ...
        {ADDIS_LOCATIONS
          .filter(location => 
            !searchedLocation || location.position.toString() !== searchedLocation.position.toString()
          )
          .map(location => (
            <Marker 
              key={location.id} 
                ...
                <div className="min-w-[200px]">
                  ...
                  {searchedLocation && (
                    routes.find(route => route.toId === location.id) && (
                      <div className="mt-2">
                        <p className="text-sm">
                          <span className="font-medium">Distance:</span> {(routes.find(route => route.toId === location.id).distance / 1000).toFixed(2)} km
                        </p>
                        <p className="text-sm">
                          <span className="font-medium">Travel time:</span> {Math.round(routes.find(route => route.toId === location.id).duration / 60)} min
                        </p>
                      </div>
                    )
                  )}
